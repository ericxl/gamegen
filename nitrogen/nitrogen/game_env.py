...
class GamepadEnv(Env):
    ...
    def __init__(
        ...
        # Falls back to a no-op stub if injection fails (e.g. games with
        # anti-cheat that block DLL injection like Overwatch). The game then
        # runs in real time and pause()/unpause() become no-ops.
        try:
            self.speedhack_client = xsh.Client(process_id=self.game_pid, arch=self.game_arch)
        except Exception as e:
            print(f"[game_env] Speedhack injection failed ({e}); running without pause/unpause.")
            class _NoopSpeedhack:
                def set_speed(self, _):
                    pass
            self.speedhack_client = _NoopSpeedhack()
